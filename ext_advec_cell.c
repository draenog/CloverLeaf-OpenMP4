...
#pragma omp for private(upwind, donor, downwind, dif, sigmat, sigma3, sigma4,\
        sigmav, sigma, sigmam, diffuw, diffdw, limiter, j)
            for (k=y_min;k<=y_max;k++) 
            {
                for (j=x_min;j<=x_max+2;j++) 
                {
                    ...
                    sigmat = fabs(vol_flux_x[FTNREF2D(j,k,x_max+5,x_min-2,y_min-2)] /
                            pre_vol[FTNREF2D(donor,k  ,x_max+5,x_min-2,y_min-2)]);
                    sigma3 = (1.0+sigmat) * (vertexdx[FTNREF1D(j,x_min-2)] /
